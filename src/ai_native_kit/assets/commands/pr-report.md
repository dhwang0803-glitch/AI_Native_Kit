...
PR 생성이 완료되면 **자동으로** 세션 회고를 실행한다.
별도 사용자 확인 없이 `/session-retro`의 Step 0~4를 수행하고 보고서를 저장한다.

> 회고 없는 PR은 학습 기회를 놓친다. PR이 "작업 완료"의 자연스러운 종착점이므로
> 여기서 자동 회고하는 것이 가장 효과적이다.
...
```
PR이 생성되었습니다. 교차 검증을 실행할까요?
Codex가 이 PR을 독립적으로 리뷰합니다. (소요: 2~5분)

1. 실행 (권장 — 코드 변경이 포함된 PR)
2. 건너뛰기 (문서만 변경한 경우)
```
...
사용자가 "실행"을 선택하면 `/cross-verify pr`을 자동 실행한다.
결과가 나오면 PR 본문에 교차 검증 요약을 추가한다.
